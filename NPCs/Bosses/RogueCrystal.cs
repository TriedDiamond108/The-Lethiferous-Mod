...
namespace TheLethiferous.NPCs.Bosses
{
	...
	public class RogueCrystal : ModNPC
	{
		...
		public override void AI()
		{
			...
			{
				frame = 0;
				MoveTowards(npc, target, (float)(distance > 300 ? 12f : 8f), 30f);
				npc.netUpdate = true;
			...
			if((double)npc.ai[0] % (Main.expertMode ? 100 : 150) == 0 && !stunned && !fastSpeed)
			{
				...
				if (attackTimer <= 2)
				{
					...
					{
						Projectile.NewProjectile(shootPos.X + (float)(-100 * npc.direction) + (float)Main.rand.Next(-41, 40), shootPos.Y - (float)Main.rand.Next(-50, 40), shootVel.X, shootVel.Y, mod.ProjectileType("LethiferousOrb"), npc.damage / 3, 5f);
					}
				} else
				{
					attackTimer = 0;
