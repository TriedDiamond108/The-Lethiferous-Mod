...
namespace TheLethiferous.NPCs.Bosses
{
	...
	public class WaspAbomination : ModNPC
	{
		private int ai;
		...
		public override void SetDefaults() 
		{
			...
			npc.value = Item.buyPrice(platinum: 999);
		...
		public override void ScaleExpertStats(int numPlayers, float bossLifeScale)
		{
			...
			npc.damage = (int)(npc.damage * 1.01f);
		}
		...
		public override void AI()
		{
			...
			{
				frame = 0;
				MoveTowards(npc, target, (float)(distance > 300 ? 30f : 29f), 30f);
				npc.netUpdate = true;
			...
			if((double)npc.ai[0] % (Main.expertMode ? 100 : 200) == 0 && !stunned && !fastSpeed)
			{
				...
				if (attackTimer <= 2)
				{
					...
					shootVel.Normalize();
					shootVel *= 20f;
					for(int i = 0; i < (Main.expertMode ? 30 : 60); i++)
					{
